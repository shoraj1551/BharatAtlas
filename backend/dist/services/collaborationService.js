...
export async function shareWorkspace(workspaceId, userEmail, role = 'viewer', requestingUserId) {
    ...
    const existingCollaborator = workspace.collaborators.find(c => c.user_id.toString() === user._id.toString());
...
export async function removeCollaborator(workspaceId, userId, requestingUserId) {
    ...
    // Remove collaborator
    workspace.collaborators = workspace.collaborators.filter(c => c.user_id.toString() !== userId.toString());
    // Log activity
    workspace.activity_log.push({
        user_id: requestingUserId,
...
export async function updatePermissions(workspaceId, userId, newRole, requestingUserId) {
    ...
    const collaborator = workspace.collaborators.find(c => c.user_id.toString() === userId.toString());
    if (!collaborator) {
