...
export async function indexPlace(place) {
    ...
    await SearchIndex.findOneAndUpdate({ place_id: place.place_id }, indexData, { upsert: true, new: true });
...
export async function advancedSearch(query, filters = {}, options = {}) {
    const { sort = 'relevance', limit = 20, offset = 0 } = options;
    // Build MongoDB query
    const mongoQuery = {};
    // Text search
    if (query && query.trim()) {
...
//# sourceMappingURL=searchIndexService.js.map