...
function enrichPlaceData(place) {
    ...
    if (place.population?.value && place.area_sq_km) {
        place.population_density = Math.round(place.population.value / place.area_sq_km);
    }
...
function calculateReadinessScore(place) {
    ...
    if (place.population_density > 500)
        scores.workforce += 10; // Dense talent pool
    else if (place.population_density > 200)
        scores.workforce += 5;
    ...
    if (place.place_type === 'city')
        scores.infrastructure += 5; // Urban bonus
    // 3. Governance (20 pts)
    const adminType = place.governance?.administration?.type || place.governance?.administrative_head || '';
    if (adminType.toLowerCase().includes('corporation'))
        scores.governance += 15;
    else if (adminType.toLowerCase().includes('municipality'))
        scores.governance += 10;
    else
        scores.governance += 5;
    if (place.governance?.government_schemes?.length > 0)
        scores.governance += 5;
    // 4. Social Fit (15 pts) - Risk reduction
    if (place.culture_society) {
        scores.social_fit += 5; // Base for data presence
        if (place.culture_society.market_adaptation_tips?.length > 0)
            scores.social_fit += 5;
        if (place.culture_society.social_norms)
            scores.social_fit += 5;
    }
    ...
    // Total Calculation
    const total = Math.round(scores.workforce +
...
export async function getPlaceById(placeId, fields = []) {
    ...
    const place = await db.collection('places')
        .findOne({ place_id: placeId }, projection.place_id ? { projection } : {});
...
//# sourceMappingURL=mongoPlaceService.js.map