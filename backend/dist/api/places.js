...
import { getAllStates, getPlaceById, getPlaceByName, searchPlaces } from '../services/mongoPlaceService.js';
...
router.patch('/:id', async (req, res, next) => {
    try {
        ...
        const place = await Place.findOneAndUpdate({ place_id: req.params.id }, { $set: req.body }, { new: true, runValidators: true });
        if (!place) {
            return res.status(404).json({
                success: false,
                error: 'Place not found'
